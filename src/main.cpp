...
#include <platform/Platform.hpp> // Include platform detection
...
bool isIOS() {
    return platform::getPlatform() == platform::PlatformType::iOS;
}
...
class $modify(EndLevelLayer) {
    ...
    void updateCoinsForIOS() {
        if (isIOS()) {
            // Custom coin replacement logic for iOS
            for (auto& coin : m_coinsToAnimate) {
                if (coin->getColor() == BRONZE_COLOR) {
                    coin->setTexture("secretCoin_2_01.png"); // Use gold coin texture
                    coin->setColor(ccWHITE); // Set color to white to show the gold coin
                }
            }
        }
    }
...
class $modify(EditorUI) {
    void onCreateObject(int objID) {
        ...
        if ((objID == USER_COIN || objID == 1614) && Mod::get()->getSettingValue<bool>("goldCoinsEditor") && CCKeyboardDispatcher::get()->getAltKeyPressed()) 
            EditorUI::onCreateObject(GOLD_COIN);
        else 
            EditorUI::onCreateObject(#include <Geode/Geode.hpp>
#include <Geode/modify/MenuLayer.hpp>
#include <Geode/modify/LoadingLayer.hpp>
#include <Geode/modify/CCSprite.hpp>
#include <Geode/modify/GameObject.hpp>
